...
graph TB
    ...
    subgraph "외부 윈도우 (별도 모듈 - 누락)"
        AISettingWin[AI 설정 창]
        ScheduleWin[스케줄 설정 창]
        ObjectSettingWin[객체 설정 창]
        SearchWin[검색 창]
        LabelingWin[라벨링 창]
        ServerSettingWin[서버 설정 창]
    end
...
**상속**: `QDialog` → `Ui_login_windows` (누락된 UI 모듈)
...
**상속**: `QMainWindow` → `Ui_MainWindow` (누락된 UI 모듈)
...
다음 모듈들은 main.py에서 import되지만 실제 파일이 누락되어 있습니다:

### 1. AI 설정 창
**모듈**: `ui.utils_ai_setting_window`
**함수**: `open_ai_setting_window(click, instance)`
**호출**: [main.py:1002](main.py#L1002)

### 2. 스케줄 설정 창
**모듈**: `ui.utils_schedule_window`
**함수**: `open_schedule_window(click, instance)`
**호출**: [main.py:1024](main.py#L1024)

### 3. 객체 설정 창
**모듈**: `ui.utils_object_setting_window`
**함수**: `open_object_setting_window(click, instance)`
**호출**: [main.py:1000](main.py#L1000)

### 4. 검색 창
**모듈**: `ui.utils_search_window`
**함수**: `open_search_window(click, instance)`
**호출**: [main.py:1023](main.py#L1023)

### 5. 라벨링 창
**모듈**: `ui.utils_labeling_window`
**함수**: `open_labeling_window(click, instance)`
**호출**: [main.py:1025](main.py#L1025)

### 6. 서버 설정 창
**모듈**: `ui.utils_server_setting_window`
**함수**: `open_server_setting_window(click, instance)`
...
이 컴포넌트 맵을 기반으로:
1. **3단계**: 기술 부채, 성능 병목, 개선 포인트 도출
2. **누락된 ui/ 모듈 복구 또는 재구현**
3. **API 명세 문서화** (AI 서버 엔드포인트 전체)
